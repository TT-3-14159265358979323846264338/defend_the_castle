...
public abstract class CoreData {
	 ...
	 * 			<br>
	 * 			timing - 発生させるタイミングコード。{@link defendthecastle.battle.Buff Buff}の発生タイミングコードで指定。Coreでは{@link defendthecastle.battle.Buff#HIT HIT}, {@link defendthecastle.battle.Buff#KILL KILL}使用不可。<br>
	 ...
	 * 			recast - スキルの再使用時間[s]。timingが{@link defendthecastle.battle.Buff#SKILL SKILL}の時のみ指定することがある。未使用なら{@link defendthecastle.battle.Buff#NONE Buff.NONE}を指定。<br>
	 * 			cost - スキルに使用するコスト。未使用なら{@link defendthecastle.battle.Buff#NONE Buff.NONE}を指定。<br>
	 * 			<br>
