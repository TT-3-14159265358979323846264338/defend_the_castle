...
public abstract class EnemyData {
	 ...
	 * 			<br>
	 * 			timing - 発生させるタイミングコード。{@link defendthecastle.battle.InternalData.Buff Buff}の発生タイミングコードで指定。Enemyでは{@link defendthecastle.battle.InternalData.Buff#SKILL SKILL}使用不可。<br>
	 * 			target - 与える対象コード。{@link defendthecastle.battle.InternalData.Buff Buff}の発生対象コードで指定。指定は敵側から見た敵味方である。<br>
	 * 			range - 与える範囲コード。{@link defendthecastle.battle.InternalData.Buff Buff}の効果範囲コードで指定。<br>
