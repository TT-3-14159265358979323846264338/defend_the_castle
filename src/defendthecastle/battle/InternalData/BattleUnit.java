...
import defendthecastle.battle.timer.TimerOperation;
...
public class BattleUnit extends BattleData{
	...
	public BattleUnit(GameTimer gameTimer, OneUnitData oneUnitData, int positionX, int positionY, ScheduledExecutorService scheduler) {
	...
	//左武器用
	public BattleUnit(GameTimer gameTimer, OneUnitData oneUnitData, ScheduledExecutorService scheduler) {
		this.gameTimer = gameTimer;
		composition = oneUnitData.getUnitDataList();
		StatusCalculation StatusCalculation = new StatusCalculation(composition);
		try {
	...
	public void install(GameData gameData, BattleUnit otherWeapon, BattleData[] unitMainData, BattleData[] facilityData, BattleData[] enemyData) {
	...
	public boolean canPossessSkill() {
	...
	protected void defeat(BattleData target) {
		int price = 60;
		defeatNumber++;
		gameData.lowMorale(defendthecastle.battle.InternalData.GameData.UNIT, price);
