...
class DrawResult extends CommonJPanel implements MouseListener{
	...
	private List<Core> getCore = new ArrayList<>();
	private List<Point> corePosition = new ArrayList<>();
	private List<Weapon> getWeapon = new ArrayList<>();
	...
	<T>boolean canGetItem(double value, List<T> lineupList, List<Double> ratioList, List<T> getList) {
		if(ratioList.size() != 0) {
	...
	void displayCore(int selectNumber) {
		new DisplayStatus().core(itemGetImage.getCoreImageList().get(getCore.get(selectNumber).getId()), getCore.get(selectNumber).getId());
	}
	
	void displayWeapon(int selectNumber) {
		new DisplayStatus().weapon(itemGetImage.getWeaponImageList().get(getWeapon.get(selectNumber).getId()), getWeapon.get(selectNumber).getId());
	}
}