...
class SaveData{
	...
	void countNumber() {
		...
		getActiveCompositionList().stream().forEach(i -> {
			core[i.getUnit(OneUnitData.CORE)]++;
			try {
				weapon[i.getUnit(OneUnitData.RIGHT_WEAPON)]++;
			}catch(Exception ignore) {
				//右武器を装備していないので、無視する
			}
			try {
				weapon[i.getUnit(OneUnitData.LEFT_WEAPON)]++;
			}catch(Exception ignore) {
				//左武器を装備していないので、無視する
			}
		});
	...
	void changeCore(int number, int selectCore) {
		getUnitData(number).setUnitData(OneUnitData.CORE, selectCore);
		countNumber();
		existsChange = true;
	}
		...
		if(Weapon.getWeaponData(selectWeapon).getHandle() == Handle.BOTH) {
			...
			case Handle.BOTH:
				if(change(number, selectWeapon) == 1) {
					getUnitData(number).setUnitData(OneUnitData.LEFT_WEAPON, Weapon.NO_WEAPON);
				}
				break;
			default:
				break;
	...
	int change(int number, int selectWeapon) {
		...
		case 0:
			...
			break;
		case 1:
			getUnitData(number).setUnitData(OneUnitData.RIGHT_WEAPON, selectWeapon);
			break;
		default:
			break;
		}
