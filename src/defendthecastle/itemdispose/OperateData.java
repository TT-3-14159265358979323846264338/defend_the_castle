...
class OperateData{
	...
	void itemCount() {
		...
		//初期武器は最低2本残さなければならない
		usedWeaponNumber[Weapon.SWORD.getId()] = initialWeaponProtection(usedWeaponNumber[Weapon.SWORD.getId()]);
		usedWeaponNumber[Weapon.BOW.getId()] = initialWeaponProtection(usedWeaponNumber[Weapon.BOW.getId()]);
	}
	...
	void countOneComposition(int index, int[] coreCount, int[] weaponCount) {
		saveComposition.getOneCompositionData(index).getOneUnitDataList().stream().forEach(i -> {
			try {
				weaponCount[i.getUnit(OneUnitData.RIGHT_WEAPON)]++;
			}catch(Exception ignore) {
				//右武器を装備していないので、無視する
			}
			coreCount[i.getUnit(OneUnitData.CORE)]++;
			try {
				weaponCount[i.getUnit(OneUnitData.LEFT_WEAPON)]++;
			}catch(Exception ignore) {
				//左武器を装備していないので、無視する
			}
		});
