...
public class FileCheck extends SQLOperation{
	 ...
	 * 初期のユニット装備
	 */
	public static final List<Integer> INITIAL_UNIT = Arrays.asList(Weapon.NO_WEAPON, Core.NORMAL_CORE.getId(), Weapon.NO_WEAPON);
	...
	void addNewUnit(Connection mysql) throws Exception{
		addNewUnitData(mysql, SaveHoldItem.CORE_NAME, Core.values().length);
		addNewUnitData(mysql, SaveHoldItem.WEAPON_NAME, Weapon.values().length);
	}
	...
	void initializeProgress(Connection mysql) throws Exception{
		int addCount = changeCount(mysql, SaveGameProgress.STAGE_NAME, Stage.values().length);
		if(addCount == 0) {
			return;
		}
