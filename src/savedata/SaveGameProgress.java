...
public class SaveGameProgress extends SQLOperation{
	...
	/**
	 * 各ステージのクリア状況。<br>
	 * {@link defaultdata.Placement#STAGE_DATA ステージ順}にクリア状況を保存。<br>
	 * 各ステージの戦功を全てクリアするとtrueになる。<br>
	 * このListのsizeは、{@link defaultdata.Placement#STAGE_DATA STAGE_DATA}に新規追加されると、{@link savedata.FileCheck FileCheck}で自動的に追加される。
	 */
	private List<Boolean> stageStatus = new ArrayList<>();
	
	/**
	 * 各ステージの戦功取得状況。<br>
	 * {@link defaultdata.Placement#STAGE_DATA ステージ順}で{@link defaultdata.stage.StageData#getMerit 戦功}クリア状況を保存。<br>
	 * このListのsizeは、{@link defaultdata.Placement#STAGE_DATA STAGE_DATA}に新規追加されると、{@link savedata.FileCheck FileCheck}で自動的に追加される。
	 */
