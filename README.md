...
test: 単体テスト用ソースフォルダ。srcと同階層にtestソースフォルダを作成し、中身を保存すること。♦現在、大規模なリファクタリング実施したため、使用不能。随時追加。♦  
...
ExecutorとFutureで使用するスレッドはScheduledExecutorServiceをmain()で定義し、これを各クラスに渡すことでリソースの節約を行った。  
...
JUnitの記載しやすさと堅牢性を考慮しながら開発した。  
・パッケージプライベートを基本とすることで、JUnitからのアクセスを可能にした。  
・インスタンス変数の定義をリターンメソッドで行うことで、スタブやOverrideによる割り込みを可能とし、テスト時に実体のクラスを使わなくて良いようにした。  
・コンストラクタの引数を増やすことで、JUnitでの依存の注入を容易にした。  
  
共通処理の抽象クラスへ移動やジェネリクスによる共通化などによって、コードの最適化を行った。  
現在も最適化できそうなコードを見つけ次第、順次リファクタリングを実施している。  
